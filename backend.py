...
async def telegram_set_webhook():
    ...
    if not webhook_url:
        print(
            "Telegram webhook skipped: set TELEGRAM_WEBHOOK_URL or RENDER_EXTERNAL_URL"
        )
        return False
...
async def event(payload: Event, request: Request):
    ...
    page_names = {
        "s1": "Page 1 — Opening",
        "s2": "Page 2 — Do you want to listen?",
        "s3": "Page 3 — Honest intro",
        "s4": "Page 4 — Feelings + photos",
        "s5": "Page 5 — Final choice",
        "s6": "Page 6 — Haan / chat",
        "s7": "Page 7 — Time chahiye",
        "game": "Secret Game — /game",
    }
    stamp = datetime.now(IST).strftime("%d %b %Y, %I:%M:%S %p")
    label = (
        LABELS.get(payload.event, "🎮 " + payload.event)
        if payload.event.startswith("game_")
        else LABELS.get(payload.event, "🔔 " + payload.event)
    )
    ...
    if not payload.notify:
        return {"ok": True, "telegram_sent": False, "silent": True}
    try:
